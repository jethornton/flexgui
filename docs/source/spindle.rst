...
If `INCREMENT` is not found, Flex will look in the .ini [DISPLAY] section for `SPINDLE_INCREMENT` and if not there will default the increment to 100 for spindle faster/slower control buttons.
...
.. note:: The digitCount property must be large enough to display the whole number.
